...
class OrchestratorAgent(BaseAgent):
    ...
    async def _select_optimal_model(self, message: str, agent_name: str) -> Optional[str]:
        ...
        try:
            ...
            import requests
            try:
                backend_url = settings.urls.backend_port
                logger.debug(f"Запрашиваем список моделей с {backend_url}/api/models")
                response = requests.get(f'{backend_url}/api/models', timeout=2)
...
Выбранная модель:"""
                ...
                logger.error(f"Убедитесь, что backend запущен на {backend_url}")
                return None
            except Exception as e:
