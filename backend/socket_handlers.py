...
async def _handle_multi_llm(
    ...
    # KB / memory_rag
    for flag, method_name, prefix in [
        (use_kb_rag, "kb_search", "База Знаний (постоянные документы)"),
        (use_memory_library_rag, "memory_rag_search", "Документы из настроек (библиотека памяти)"),
    ]:
        if flag and rag_client:
            try:
                hits = await getattr(rag_client, method_name)(user_message, k=8)
                if hits:
                    parts, total = [], 0
                    for i, (content, score, doc_id, chunk_idx) in enumerate(hits, 1):
                ...
                logger.error(f"multi-llm {method_name}: {e}")
...
async def _handle_agent_mode(
    ...
    for flag, method_name, prefix in [
        (use_kb_rag, "kb_search", "База Знаний (документы)"),
        (use_memory_library_rag, "memory_rag_search", "Документы из настроек (библиотека памяти)"),
    ]:
        if flag and rag_client:
            try:
                hits = await getattr(rag_client, method_name)(user_message, k=6) or []
                if hits:
                    parts, tl = [], 0
                ...
                logger.error(f"Agent {method_name}: {e}")
