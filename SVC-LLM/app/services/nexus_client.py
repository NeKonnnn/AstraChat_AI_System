...
class NexusClient:
    ...
    def download_artifact(self, destination_path: str) -> bool:
        ...
        if not self.is_enabled():
            logger.info("Загрузка из Nexus отключена")
            return False
        if not self.settings.url:
            logger.info("Не все параметры Nexus заданы")
            return False
        try:
            logger.info(f"Загрузка артефакта из Nexus: {self.settings.url}")
            response = self.session.get(self.settings.url, stream=True)
            ...
            with open(destination_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=8192):
                    f.write(chunk)
            logger.info(f"Артефакт успешно загружен в: {destination_path}")
            return True
        except Exception as e:
            logger.error(f"Ошибка при загрузке артефакта из Nexus: {str(e)}")
            return False
