...
async def list_models(
    llama_service: BaseLLMHandler = Depends(get_llm_handler),
):
    """Список доступных моделей. Не требует загруженной модели (не отдаёт 503 после сбоя загрузки)."""
    ...
    if not models_list and isinstance(llama_service, LlamaHandler) and llama_service.model_name:
        models_list.append({
...
async def load_model(
    ...
    """Загрузить/переключить модель по имени (имя файла без .gguf). Поддерживается только для llama.cpp backend. Вызывается даже если текущая модель не загружена (после сбоя)."""
    if not isinstance(llama_service, LlamaHandler):
        raise HTTPException(
            status_code=501,
            detail="Dynamic model switching is only supported for llama.cpp backend"
        )
    model_name = (request.model or "").strip()
    if not model_name:
        raise HTTPException(status_code=400, detail="model name is required")
    try:
        success = await llama_service.load_model_by_name(model_name)
        if success:
