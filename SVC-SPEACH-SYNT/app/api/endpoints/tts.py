            ...
            try:
                ...
                try:
                    ...
                    if simplified_chunk.strip():
                        ...
                        # Повторная обработка скорости для fallback
                        if speech_rate != 1.0:
                            if len(audio.shape) > 1:
                                audio = audio.squeeze()
                            original_length = len(audio)
...
async def tts_health_check():
    ...
    try:
        ...
        models = await get_silero_handler()
        # Исправлена ошибка: models может быть None
        loaded_keys = list(models.keys()) if models else []
